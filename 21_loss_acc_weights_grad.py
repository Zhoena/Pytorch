...
train_transform = transforms.Compose([
    ...
    transforms.RandomCrop(32, padding=4),  # 对图像随机裁剪
...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.1)  # 等间隔调整
...
for epoch in range(MAX_EPOCH):
    ...
    for i, data in enumerate(train_loader):
        ...
        # 统计分类情况
        _, predicted = torch.max(outputs.data, 1)  # 取其行方向的最大值
        # print("outputs:{}".format(outputs.data))
        # print("_______:", _)
        # print("predicted:", predicted)
        total += labels.size(0)  # 统计元素个数
        # print("total:", total)
        ...
        if (i+1) % log_interval == 0:  # 每十个打印平均损失
