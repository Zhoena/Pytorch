...
if __name__ == "__main__":
    ...
    model_path = "./32_checkpoint_14_epoch.pkl"  # 29_finetune_resnet18.py里保存的
    time_total = 0
    ...
    resnet18.to(device)
    resnet18.eval()  # 必须设置这一步，表示不是在训练状态，所以inference必须有这一步

    with torch.no_grad():  # inference也必须有这一步，不计算梯度，提高运算速度
        for idx, img_name in enumerate(img_names):
            ...
            img_tensor.unsqueeze_(0)  # 在第0个维度增加一个维度，变成4D张量
            img_tensor = img_tensor.to(device)  # 等号需要自己手动赋值
    ...
    print("\ndevice:{} total time:{:.1f}s mean:{:.3f}s".format(device, time_total, time_total / num_img))
