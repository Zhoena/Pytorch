...
from tools.public_my_dataset import AntsDataset
...
if flag:
    ...
    resnet18_ft.load_state_dict(state_dict_load)  # 将原任务中的学习参数放到新任务的模型当中
...
if flag_m1:
    for param in resnet18_ft.parameters():
        param.requires_grad = False  # 不求梯度，就不会更新了
    print("conv1.weights[0, 0, ...]:\n {}".format(resnet18_ft.conv1.weight[0, 0, ...]))


# 3/3 替换fc层
num_ftrs = resnet18_ft.fc.in_features  # 从原始的resnet18_ft.fc中获取input features数
resnet18_ft.fc = nn.Linear(num_ftrs, classes)  # 输入，输出，再赋值给resnet18_ft.fc，即完成了resnet18_ft的fc层替换

resnet18_ft.to(device)  # 模型放到gpu或cpu
...
for epoch in range(start_epoch + 1, MAX_EPOCH):
    ...
    for i, data in enumerate(train_loader):
        ...
        # forward
        inputs, labels = data
        inputs, labels = inputs.to(device), labels.to(device)  # 训练数据也放到gpu或cpu
