...
if flag:
    ...
    inputs = torch.linspace(-3, 3, steps=500)  # 从（-3，3）平均取500个数据点
...
# --------------------- compute by hand
    ...
    # 这里要用上面定义的inputs，target，如果重新定义的话，新产生的随机数与上面的不同，得不到相同的结果
    idx = 0
...
if flag:
    ...
    y = torch.tensor([[0, 3, -1, -1]], dtype=torch.long)  # 缺少的部分用 -1 占位
